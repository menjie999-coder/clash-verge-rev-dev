  ...
  const proxyColItemsMemo = useMemo(() => {
    ...
    return proxyCol.map((proxyItem) => (
      <ProxyItemMini
        ...
        inChain={!!proxyItem && (chainNames?.has(proxyItem.name) ?? false)}
        showType={showType}
        onClick={() => onChangeProxy(group, proxyItem!)}
      />
    ))
  }, [type, proxyCol, item.key, group, showType, onChangeProxy, chainNames])
  ...
  if (type === 2) {
    return (
      <ProxyItem
        ...
        inChain={!!proxy && (chainNames?.has(proxy.name) ?? false)}
